...
export default function SupervisorDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true)
        const res = await fetch('/api/supervisor/dashboard', { signal: controller.signal })
        if (res.status === 401) {
    ...
    return (
      <div className="p-6 bg-background min-h-full">
        <div className="bg-surface border border-border p-12 text-center">
          ...
          <button onClick={() => setRetryKey(k => k + 1)} className="mt-3 text-xs text-primary hover:text-primary-hover font-medium">
            Retry
          </button>
        </div>
      </div>
