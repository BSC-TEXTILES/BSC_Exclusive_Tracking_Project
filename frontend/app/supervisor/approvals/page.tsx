...
export default function ApprovalsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(`/api/supervisor/approvals?${params.toString()}`, { signal: controller.signal })
        if (res.status === 401) {
        ...
        if (!cancelled) setError('Network error loading approvals')
      } finally {
  ...
  }, [userFilter, moduleFilter, statusFilter, dateFrom, dateTo, refreshKey])
